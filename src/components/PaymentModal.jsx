...
export default function PaymentModal({ isOpen, onClose, onConfirm, client, bcvRate, paymentConfig }) {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      triggerHaptic('heavy');
      // Set completed payment to display receipt
      setCompletedPayment({
  ...
  if (completedPayment) {
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/85 backdrop-blur-sm animate-in fade-in">
        <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-2xl text-center relative">
          
          <div className="w-14 h-14 rounded-2xl bg-emerald-500/15 text-emerald-400 border border-emerald-500/30 flex items-center justify-center mx-auto mb-4">
          ...
          <p className="text-xs text-slate-400 mt-1">
            Se renovó la suscripción de <strong className="text-slate-200">{client.nombre_negocio}</strong> hasta el <strong className="text-emerald-400">{completedPayment.validoHasta}</strong>.
          </p>

          {/* Receipt Preview Box */}
          <div className="mt-5 p-4 rounded-2xl bg-slate-950/80 border border-slate-800 text-left text-xs font-mono space-y-1 text-slate-300">
            <div className="flex justify-between font-bold text-white border-b border-slate-800/80 pb-1.5 mb-2">
          ...
          <div className="mt-6 flex flex-col gap-2.5">
            <a
              ...
              className="w-full py-3 px-4 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-semibold text-xs transition-all shadow-lg shadow-emerald-900/30 flex items-center justify-center gap-2 active:scale-95"
            >
            ...
            <div className="grid grid-cols-2 gap-2">
              <button
                ...
                className="py-2.5 px-3 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-medium border border-slate-700 transition-colors flex items-center justify-center gap-1.5"
              >
                {copiedReceipt ? (
                  <>
              ...
              <button
                ...
                className="py-2.5 px-3 rounded-xl border border-slate-700 hover:bg-slate-800 text-slate-300 text-xs font-medium transition-colors"
              >
                Finalizar
              </button>
            </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm animate-in fade-in">
      <div className="w-full max-w-lg bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-7 shadow-2xl relative max-h-[92vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-2 gap-1.5 mt-1.5">
            ...
            <div className="relative">
              <input
                ...
                className="w-full py-1.5 px-3 rounded-xl bg-slate-950/60 border border-slate-800 text-xs text-slate-200 placeholder-slate-500 focus:outline-none focus:border-emerald-500"
              />
            </div>
          </div>
        </div>
        ...
        <div className="mt-4 p-4 rounded-2xl bg-slate-950/70 border border-slate-800 space-y-2.5">
        ...
        <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-2.5">
          <div>
            ...
            <input
              ...
              className="w-full py-1.5 px-3 rounded-xl bg-slate-950/60 border border-slate-800 text-xs text-slate-200 placeholder-slate-500 focus:outline-none focus:border-emerald-500 font-mono"
            />
          </div>
          <div>
            <label className="block text-[11px] font-semibold text-slate-400 mb-1">
        ...
        <div className="mt-4 p-3 rounded-xl bg-slate-800/40 border border-slate-800 flex items-center justify-between text-xs">
