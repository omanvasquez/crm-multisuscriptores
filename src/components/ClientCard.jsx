  ...
  return (
    <div className={`rounded-2xl border transition-all duration-200 relative group overflow-hidden ${
      isSuspended
        ...
        : isOverdue 
          ? isTrialExpired 
            ? 'bg-amber-50/70 dark:bg-amber-950/15 border-amber-300 dark:border-amber-500/40 hover:border-amber-400 dark:hover:border-amber-500/60 shadow-md shadow-amber-900/10'
            : 'bg-rose-50/70 dark:bg-rose-950/20 border-rose-300 dark:border-rose-500/40 hover:border-rose-400 dark:hover:border-rose-500/60 shadow-md shadow-rose-900/10' 
          : isUpcoming
      ...
      <div className="p-5">
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap mb-1.5">
              ...
              {isSuspended ? (
                <span className="text-[11px] font-semibold px-2.5 py-0.5 rounded-full bg-slate-200 dark:bg-slate-800 text-slate-600 dark:text-slate-400 border border-slate-300 dark:border-slate-700 flex items-center gap-1">
                  <PauseCircle className="w-3 h-3 text-slate-500 dark:text-slate-400" />
                  Suspendido
                </span>
              ) : isTrial ? (
                <span className="text-[11px] font-semibold px-2.5 py-0.5 rounded-full bg-purple-500/15 text-purple-700 dark:text-purple-300 border border-purple-500/30 flex items-center gap-1">
                  <Sparkles className="w-3 h-3 text-purple-600 dark:text-purple-400" />
                  En Prueba ({client.dias_restantes_prueba !== null ? `${client.dias_restantes_prueba}d` : 'Activa'})
                </span>
              ) : isTrialExpired ? (
                <span className="text-[11px] font-semibold px-2.5 py-0.5 rounded-full bg-amber-500/15 text-amber-700 dark:text-amber-300 border border-amber-500/30 flex items-center gap-1">
                  <Clock className="w-3 h-3 text-amber-600 dark:text-amber-400" />
                  Prueba Vencida
                </span>
              ) : estadoCliente === 'MOROSO' ? (
                <span className="text-[11px] font-semibold px-2.5 py-0.5 rounded-full bg-rose-500/10 text-rose-700 dark:text-rose-400 border border-rose-500/30 flex items-center gap-1">
                ...
                <span className="text-[11px] font-semibold px-2.5 py-0.5 rounded-full bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border border-emerald-500/30 flex items-center gap-1">
            ...
            <h3 className="text-lg font-bold text-slate-900 dark:text-white tracking-tight truncate" title={client.nombre_negocio}>
            ...
            {client.id_externo ? (
              <div className="mt-1 flex items-center gap-1.5">
                <button
                  ...
                  className="inline-flex items-center gap-1 text-[11px] font-mono px-2 py-0.5 rounded-md bg-slate-100 hover:bg-slate-200 dark:bg-slate-950/80 dark:hover:bg-slate-800 border border-slate-200 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-700 text-slate-700 dark:text-slate-300 transition-colors group/id"
                >
                  <KeyRound className="w-3 h-3 text-slate-500 group-hover/id:text-emerald-600 dark:group-hover/id:text-emerald-400" />
                  ...
                  {copiedId ? (
                    ...
                    <Copy className="w-3 h-3 text-slate-400 dark:text-slate-500 opacity-60 group-hover/id:opacity-100 ml-0.5" />
                  )}
                </button>
                {copiedId && (
        ...
        <div className="mt-4 space-y-1.5 text-xs text-slate-500 dark:text-slate-400">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="truncate">
              ...
              <span className="text-slate-700 dark:text-slate-300 font-medium">{client.estado_region || 'Cojedes'}</span>
            </span>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            {isSuspended ? (
              <span className="text-slate-500 dark:text-slate-400">
                Estado: <strong className="text-slate-700 dark:text-slate-300">Servicio suspendido en app</strong>
              </span>
            ) : isTrial ? (
              <span>
                Prueba hasta: <strong className="text-purple-600 dark:text-purple-300">
                  {trialEndDate ? trialEndDate.toLocaleDateString('es-VE') : 'En curso'}
                </strong>
              </span>
            ) : isTrialExpired ? (
              <span>
                Prueba finalizada: <strong className="text-amber-600 dark:text-amber-400">
                  Requiere cobro del 1er mes
                </strong>
              </span>
            ) : (
              <span>
                Próximo corte: <strong className={isOverdue ? 'text-rose-600 dark:text-rose-400' : isUpcoming ? 'text-amber-600 dark:text-amber-300' : 'text-slate-700 dark:text-slate-200'}>
        ...
        {/* Pricing Banner */}
        <div className="mt-5 p-3 rounded-xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800/80 flex items-center justify-between">
          <div>
            ...
            <div className="flex items-baseline gap-1">
              <span className="text-xl font-extrabold text-slate-900 dark:text-white">${tarifaUsd.toFixed(2)}</span>
              <span className="text-[10px] text-slate-500 dark:text-slate-400">USD</span>
            </div>
          </div>
          <div className="text-right">
            <span className="text-[10px] text-slate-500 dark:text-slate-400 uppercase font-semibold">Monto en Bolívares</span>
        ...
        <div className="mt-4 grid grid-cols-2 gap-2">
          ...
          <a
            ...
            className={`flex items-center justify-center gap-1.5 py-2.5 px-3 rounded-xl font-semibold text-xs transition-all active:scale-95 ${
              isSuspended
                ...
                : isOverdue
                  ? isTrialExpired
                    ? 'bg-amber-600 hover:bg-amber-500 text-white shadow-md shadow-amber-900/20 ring-2 ring-amber-500/50'
                    : 'bg-emerald-600 hover:bg-emerald-500 text-white shadow-md shadow-emerald-900/20 ring-2 ring-emerald-500/50'
                  : isUpcoming
                    ? 'bg-amber-500 hover:bg-amber-600 text-slate-950 font-bold shadow-md shadow-amber-900/10'
          ...
          <button
            ...
            className={`flex items-center justify-center gap-1.5 py-2.5 px-3 rounded-xl font-semibold text-xs shadow-md transition-all active:scale-95 text-white ${
              isSuspended
                ? 'bg-gradient-to-r from-slate-600 to-emerald-600 hover:from-slate-500 hover:to-emerald-500 shadow-slate-900/10'
                : isTrialExpired 
